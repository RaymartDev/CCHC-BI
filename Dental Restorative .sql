...
;WITH department AS (
	...
	SELECT DISTINCT
		...
		p.age,
		CASE
			WHEN p.age BETWEEN 0 AND 5 THEN 'A. 0 to 5'
			WHEN p.age BETWEEN 6 AND 13 THEN 'B. 6 to 13'
			WHEN p.age BETWEEN 14 AND 21 THEN 'C. 14 to 21'
			WHEN p.age >= 22 THEN 'D. 22 and Up'
			ELSE 'NA'
		END AS [Age Range],
		chart.CHART_STATUS,
	...
	WHERE 
		...
		AND (cp.ADACODE LIKE 'D21%' OR cp.ADACODE LIKE 'D23%')
		AND HISTORY = 0
), final AS (
SELECT
	da.Provider,
	...
	da.[Age Range] AS [Age Range],
	da.dos AS DOS,
	IIF(n.PATID IS NULL, 'N', 'Y') AS [Restorative Treatment]
FROM
	(SELECT * FROM dentalAppt WHERE rn = 1) da
	LEFT JOIN (SELECT * FROM numerator WHERE rn = 1) n ON da.PATID = n.PATID
)
SELECT * FROM final
ORDER BY Provider, [Age Range],[Patient ID]