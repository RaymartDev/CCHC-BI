...
;WITH department AS (
	...
	WHERE 
		...
		AND TRIM(cp.ADACODE) IN ('D0120', 'D0145', 'D0150')
		AND HISTORY = 0
		AND p.age BETWEEN 10 AND 20
),numerator AS (
	SELECT DISTINCT
		chart.PATID,
		cp.ADACODE,
...
SELECT
	da.Provider,
	da.Clinic,
	da.PatExtID AS [Patient ID],
	da.age AS Age,
	da.ADACODE AS [Procedure Code],
	da.dos AS DOS,
	IIF(n.PATID IS NULL, 'N', 'Y') AS [Caries Risk Assesed]
FROM
