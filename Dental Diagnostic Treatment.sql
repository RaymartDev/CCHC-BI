...
;WITH department AS (
	...
	WHERE 
		...
		AND TRIM(cp.ADACODE) IN ('D0120')
		AND HISTORY = 0
		AND p.age BETWEEN 4 AND 20
),numerator AS (
	SELECT DISTINCT
		chart.PATID,
		cp.ADACODE,
	...
	WHERE 
		...
		AND (cp.ADACODE IN ('D0210', 'D0220', 'D0230', 'D0270', 'D0272', 'D0274'))
		AND HISTORY = 0
), final AS (
SELECT
	da.Provider,
	...
	da.age AS Age,
	da.dos AS DOS,
	ISNULL(n.ADACODE, '') AS [X-ray Code],
	IIF(n.PATID IS NULL, 'N', 'Y') AS [X-ray Completed]
FROM
